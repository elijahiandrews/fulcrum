  ...
  return (
    <main className="container" style={{ padding: "2rem 0 3rem 0" }}>
      ...
      <section className="card" style={{ marginBottom: "1rem" }}>
        ...
        {liveStatus.note ? <p style={{ marginBottom: 0, color: "var(--muted2)" }}><strong>Note:</strong> {liveStatus.note}</p> : null}
      </section>
      ...
      <section className="card" style={{ marginBottom: "1rem" }}>
        ...
        <p style={{ margin: "0.65rem 0 0.25rem 0", color: "var(--muted)" }}>Low-confidence symbols (&lt;70%)</p>
        {lowConfidence.length === 0 ? (
          <p style={{ marginTop: 0, color: "var(--muted2)" }}>No low-confidence symbols in current snapshot.</p>
        ) : (
          lowConfidence.map((row) => (
            <p key={`low-${row.symbol}`} style={{ margin: "0.2rem 0", color: "var(--text)" }}>
              <strong>{row.symbol}</strong> - {row.confidence.toFixed(0)}% confidence - origin {row.dataOrigin}
            </p>
          ))
        )}
        <p style={{ margin: "0.65rem 0 0.25rem 0", color: "var(--muted)" }}>Fallback/proxy-dependent symbols</p>
        {fallbackHeavy.length === 0 ? (
          <p style={{ marginTop: 0, color: "var(--muted2)" }}>No fallback-heavy symbols in current snapshot.</p>
        ) : (
          fallbackHeavy.map((row) => (
            <p key={`fallback-${row.symbol}`} style={{ margin: "0.2rem 0", color: "var(--text)" }}>
      ...
      <section className="card" style={{ marginBottom: "1rem" }}>
        ...
        <p style={{ marginBottom: "0.35rem", color: "var(--muted)" }}>Recent score jumps / drops</p>
        {scoreChanges.length === 0 ? (
          <p style={{ marginTop: 0, color: "var(--muted2)" }}>No material score changes captured yet.</p>
        ) : (
          scoreChanges.map((event, idx) => (
            <p key={`score-change-${idx}`} style={{ margin: "0.2rem 0", color: "var(--text)" }}>
              <strong>{event.symbol}</strong> - {event.message}
            </p>
          ))
        )}
        <p style={{ margin: "0.75rem 0 0.35rem 0", color: "var(--muted)" }}>Recent catalyst changes</p>
        {catalystChanges.length === 0 ? (
          <p style={{ marginTop: 0, color: "var(--muted2)" }}>No catalyst transitions captured yet.</p>
        ) : (
          catalystChanges.map((event, idx) => (
            <p key={`catalyst-change-${idx}`} style={{ margin: "0.2rem 0", color: "var(--text)" }}>
              <strong>{event.symbol}</strong> - {event.message}
            </p>
          ))
        )}
      ...
      <section className="card">
        ...
        <p style={{ marginBottom: "0.35rem", color: "var(--muted)" }}>High-severity alerts (critical/high)</p>
        {highSeverityAlerts.length === 0 ? (
          <p style={{ marginTop: 0, color: "var(--muted2)" }}>No high-severity alerts in memory.</p>
        ) : (
          highSeverityAlerts.map((alert) => (
            <p key={`sev-${alert.id}`} style={{ margin: "0.2rem 0", color: "var(--text)" }}>
        ...
        <p style={{ margin: "0.75rem 0 0.35rem 0", color: "var(--muted)" }}>Downgraded / resolved alerts</p>
        {downgradedOrResolved.length === 0 ? (
          <p style={{ marginTop: 0, color: "var(--muted2)" }}>No downgraded or resolved alerts in memory.</p>
        ) : (
          downgradedOrResolved.map((alert) => (
            <p key={`state-${alert.id}`} style={{ margin: "0.2rem 0", color: "var(--text)" }}>
